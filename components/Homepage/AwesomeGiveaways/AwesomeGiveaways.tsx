import React from 'react'

const AwesomeGiveaways = () => {
  return (
    <div className="font-jost bg-[url('/assets/homepage/awesomegiveaways/background.png')] bg-cover">
      <div className=' mx-auto max-w-7xl text-white py-8'>
        <h1 className=" font-jost text-4xl font-semibold text-center py-8">Awesome Giveaways</h1>
        <div className=' max-w-7xl mx-auto my-4 p-4 bg-yellow-secondary'>
          <div className='flex flex-wrap'>
            <div className='flex flex-col w-1/5 px-4 '>
              <div className='bg-white shadow-lg rounded-md'>
                <div className=' border-b border-grey-primary '>
                  <img src="./assets/homepage/producttracker/image.png" alt="" className=' mx-auto' />
                </div>
                <div className=''>
                  <p className=' font-medium text-xl text-black-100 py-2 px-2'>Rainbow Secret Mewtwo and Mew PSA 9</p>
                  <span className=' text-grey-primary py-2 px-2'>Ends in: 5 days</span>
                </div>
              </div>
            </div>
            <div className='flex flex-col w-1/5 px-4 '>
              <div className='bg-white shadow-lg rounded-md'>
                <div className=' border-b border-grey-primary '>
                  <img src="./assets/homepage/producttracker/image.png" alt="" className=' mx-auto' />
                </div>
                <div className=''>
                  <p className=' font-medium text-xl text-black-100 py-2 px-2'>Rainbow Secret Mewtwo and Mew PSA 9</p>
                  <span className=' text-grey-primary py-2 px-2'>Ends in: 5 days</span>
                </div>
              </div>
            </div>
            <div className='flex flex-col w-1/5 px-4 '>
              <div className='bg-white shadow-lg rounded-md'>
                <div className=' border-b border-grey-primary '>
                  <img src="./assets/homepage/producttracker/image.png" alt="" className=' mx-auto' />
                </div>
                <div className=''>
                  <p className=' font-medium text-xl text-black-100 py-2 px-2'>Rainbow Secret Mewtwo and Mew PSA 9</p>
                  <span className=' text-grey-primary py-2 px-2'>Ends in: 5 days</span>
                </div>
              </div>
            </div>
            <div className='flex flex-col w-1/5 px-4 '>
              <div className='bg-white shadow-lg rounded-md'>
                <div className=' border-b border-grey-primary '>
                  <img src="./assets/homepage/producttracker/image.png" alt="" className=' mx-auto' />
                </div>
                <div className=''>
                  <p className=' font-medium text-xl text-black-100 py-2 px-2'>Rainbow Secret Mewtwo and Mew PSA 9</p>
                  <span className=' text-grey-primary py-2 px-2'>Ends in: 5 days</span>
                </div>
              </div>
            </div>
            <div className='flex flex-col w-1/5 px-4 '>
              <div className='bg-white shadow-lg rounded-md'>
                <div className=' border-b border-grey-primary '>
                  <img src="./assets/homepage/producttracker/image.png" alt="" className=' mx-auto' />
                </div>
                <div className=''>
                  <p className=' font-medium text-xl text-black-100 py-2 px-2'>Rainbow Secret Mewtwo and Mew PSA 9</p>
                  <span className=' text-grey-primary py-2 px-2'>Ends in: 5 days</span>
                </div>
              </div>
            </div>
          </div>
          
        </div>
        <div>
          <button className=" border bg-white text-black-100 font-jost text-xl rounded-md px-16 py-2">View All</button>
        </div>
      </div>
    </div>
  )
}

export default AwesomeGiveaways